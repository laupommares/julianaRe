                ...
                <div class="flex items-center justify-center">
                    <div class="w-full flex flex-col justify-center items-center my-10 text-dark gap-8">
                        ...
                        <p class="text-base font-light leading-8 text-center">
                            En esta sección encontrarás recetas fáciles, saludables y deliciosas, junto con prácticos tips de cocina para que disfrutes de una alimentación nutritiva y equilibrada en tu día a día. ¡Descubrí cómo preparar comidas que cuiden tu cuerpo sin perder sabor!
                        </p>
                        ...
                        <div class="carousel w-full">
                            ...
                            @foreach ($chunks as $index => $pair)
                                <div id="slide{{ $index + 1 }}" class="carousel-item relative w-full flex gap-4 justify-center">
                                    @foreach ($pair as $recipe)
                                        <livewire:components.cards.card 
                                            ...
                                            :link="route('recipes.show', ['slug' => $recipe->slug])"
                                            :slug="$recipe->slug"
                                            :route="'recipes.show'" 
                                        />
                                    @endforeach
                                    ...
                                    <div class="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
                                        <a href="#slide{{ $index == 0 ? count($chunks) : $index }}" class="btn bg-orange border-none">
                                            <span class="material-symbols-outlined text-dark" style="transform: scaleX(-1) scale(1.5, 1); font-size: 32px;">
                                                trending_flat
                                            </span>
                                        </a>
                                        <a href="#slide{{ ($index + 2) > count($chunks) ? 1 : ($index + 2) }}" class="btn bg-orange border-none">
                                            <span class="material-symbols-outlined scale-x-150 text-dark" style="font-size: 32px;">
                                                trending_flat
                                            </span>  
                                        </a>
                                    </div>
